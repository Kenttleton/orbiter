...
fn host_run_command(cmd: &str, args: &[&str]) -> String {
    ...
    let n = unsafe {
        run_command(spec.as_ptr(), spec.len() as u32, out.as_mut_ptr(), out.len() as u32)
    };
...
fn write_state_report(r: &StateReport) {
    let mut buf = format!(
        ...
        r.present, r.reachable, r.in_path, json_str(&r.manager),
    );
...
fn write_detect_report(detected: bool, role: &str, brand: &str, version: &str) {
    ...
    let buf = format!(
        ...
        json_str(role), json_str(brand), json_str(version),
    );
    host_write_output(buf.as_bytes());
}
...
pub extern "C" fn detect() {
    ...
    write_detect_report(true, "tool", "git", &parse_git_version(&version_out));
}
...
#[no_mangle]
pub extern "C" fn scan() { initialize(); }
