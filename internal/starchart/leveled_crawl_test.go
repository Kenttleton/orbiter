...
func TestLeveledBranchCrawl_IncludesCallsignOnlyLevels(t *testing.T) {
	sc := testDB(t)
	ctx := context.Background()
	...
	// Galaxy level has no resources but has callsign transponders — must be included.
	...
	assert.Len(t, lb.Levels[1].Transponders, 1)
}
